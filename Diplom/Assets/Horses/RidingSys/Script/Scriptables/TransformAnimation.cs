...
{
    [CreateAssetMenu(menuName = "RidingSystem/Extras/Anim Transform", order = 2100)]
        ...
        internal IEnumerator PlayTransformAnimation(Transform item, float time, float delay)
        {
            if (item != null)
            {
                ...
                while ((time > 0) && (elapsedTime <= time) && item != null)
                {
                    ...
                    float resultPos = PosCurve.Evaluate(elapsedTime / time);
                    float resultRot = RotCurve.Evaluate(elapsedTime / time);
                    float resultSca = ScaleCurve.Evaluate(elapsedTime / time);
