...
{
    [AddComponentMenu("RidingSystem/Utilities/Colliders/Trigger Proxy")]
    public class TriggerProxy : MonoBehaviour, IMLayer
    {
        ...
        public bool TrueConditions(Collider other)
        {
            ...
            if (transform.IsChildOf(other.transform)) return false;
            if (other.transform.IsChildOf(Owner)) return false;
        ...
        public GameObject FindRealParent(Transform other)
        {
            ...
            else
            {
                ...
                {
                    if (MTools.Layer_in_LayerMask(other.parent.gameObject.layer, Layer))
                    {
                        return FindRealParent(other.parent);
                    }
                    else
                    {
                        return other.gameObject;
        ...
        public void OnTriggerEnter(Collider other)
        {
            ...
            {
                var realRoot = other.transform.root.gameObject;

                if (!MTools.Layer_in_LayerMask(realRoot.layer, Layer))
                    realRoot = FindRealParent(other.transform);

                OnTrigger_Enter.Invoke(other);

                if (m_colliders.Find(coll => coll == other) == null)
                {
        ...
        public void TriggerExit(Collider other, bool remove)
        {
            if (TrueConditions(other))
            {
                ...
                if (EnteringGameObjects.Contains(realRoot)) 
                {
                    if (!m_colliders.Exists(c => c != null && c.transform.root.gameObject == realRoot)) 
                    {
        ...
        private void OnDisable()
        {
            ...
            if (EnteringGameObjects.Count > 0)
            {
                foreach (var c in EnteringGameObjects)
                {
                    ...
                    if (c) OnGameObjectExit.Invoke(c);
                }
            }
